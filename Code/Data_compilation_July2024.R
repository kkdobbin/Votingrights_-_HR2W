#Revised data compilation using 2024 needs assessment
...
CWSgov <- read.csv(here::here("Data_raw/ACTIVE VERSION_ CWS institution types Sept 28 2023.csv"), header=T,
                   na.strings=c("","NA"))
...
#make factor
CWSgov$Residential <- as.factor(CWSgov$Residential)

#Make a second version considering MHPs as ancillary (excluding them from consideration) to compare results as a sensitivity test later
CWSgov$Residential_noMHP <- ifelse(CWSgov$Final_inst_update == "Community Services District" |
                                     CWSgov$Final_inst_update == "County Service Area" |
                                     CWSgov$Final_inst_update == "Municipal Water District" |
                                     CWSgov$Final_inst_update == "City" |
                                     CWSgov$Final_inst_update == "County Sanitation District"|
                                     ...
                                     CWSgov$Final_inst_update == "Joint Powers Authority/Agreement" |
                                     CWSgov$Final_inst_update == "Investor Owned Utility" |
                                     CWSgov$Final_inst_update == "California Water District", "Yes", NA)

CWSgov$Residential_noMHP <- ifelse(CWSgov$Final_inst_update == "County" | 
                                     CWSgov$Final_inst_update == "Federal" | 
                                     CWSgov$Final_inst_update == "Private - Ancillary" |
                                     CWSgov$Final_inst_update == "School District" |
                                     CWSgov$Final_inst_update == "State" |
                                     CWSgov$Final_inst_update == "Mobile Home Park",
                                   "No", CWSgov$Residential_noMHP)

CWSgov$Residential_noMHP <- as.factor(CWSgov$Residential_noMHP)
...
#remove non-residential systems for this analysis (will do a sensitivity test later to assess the impact of including MHPs in the analysis)
CWSgov <- filter(CWSgov, Residential== "Yes")
...
CWSgov <- filter(CWSgov, Changed_enfranchisement. == "No" | is.na(Changed_enfranchisement.)) # removes 20 systems with institutional changes that correspond to enfranchisement changes since 2018
...
## SAFER
SAFER2024 <- read.csv(here::here("Data_raw/SaferRiskAssessmentTable2024.csv"))#SAFER data comes from https://data.ca.gov/dataset/safer-failing-and-at-risk-drinking-water-systems. Dictionary saved in data_raw folder as well. Is from 2024 report. 
...
Data <- Data %>%
  mutate(CONSTITUENTS_OF_EMERGING_CONCERN_RISK_LEVEL_BI = as.factor(ifelse(CONSTITUENTS_OF_EMERGING_CONCERN_RISK_LEVEL == "HIGH", "HIGH", "NONE")),
         NUMBER_OF_WATER_SOURCES_RISK_LEVEL_BI = as.factor(ifelse(NUMBER_OF_WATER_SOURCES_RISK_LEVEL == "NONE", "NONE", "HIGH")),
         BOTTLED_WATER_OR_HAULED_WATER_RELIANCE_RISK_LEVEL = recode(BOTTLED_WATER_OR_HAULED_WATER_RELIANCE_RISK_LEVEL, "VERY HIGH" = "HIGH"),
         PERCENT_OF_MEDIAN_HOUSEHOLD_INCOME_MHI_RISK_LEVEL_BI = as.factor(ifelse(PERCENT_OF_MEDIAN_HOUSEHOLD_INCOME_MHI_RISK_LEVEL == "NONE", "NONE", "HIGH")))
...
WQ <- Data %>% 
  ...
  mutate(HISTORY_OF_E_COLI_PRESENCE_RISK_LEVEL = case_when(HISTORY_OF_E_COLI_PRESENCE_RISK_LEVEL == "HIGH" ~ 1,
         ...
         CONSTITUENTS_OF_EMERGING_CONCERN_RISK_LEVEL_BI = case_when(CONSTITUENTS_OF_EMERGING_CONCERN_RISK_LEVEL_BI == "HIGH" ~ 1,
...
Accessibility <- Data %>% 
  ...
  mutate(NUMBER_OF_WATER_SOURCES_RISK_LEVEL_BI = case_when(NUMBER_OF_WATER_SOURCES_RISK_LEVEL_BI == "HIGH" ~ 1,
        ...
        BOTTLED_WATER_OR_HAULED_WATER_RELIANCE_RISK_LEVEL = case_when(BOTTLED_WATER_OR_HAULED_WATER_RELIANCE_RISK_LEVEL == "HIGH" ~ 1,
                                                                       BOTTLED_WATER_OR_HAULED_WATER_RELIANCE_RISK_LEVEL == "NONE" ~ 0),
        SOURCE_CAPACITY_VIOLATION_RISK_LEVEL = case_when(SOURCE_CAPACITY_VIOLATION_RISK_LEVEL == "HIGH" ~ 1,
...
Affordability <- Data %>% 
  ...
  mutate(PERCENT_OF_MEDIAN_HOUSEHOLD_INCOME_MHI_RISK_LEVEL_BI = case_when(PERCENT_OF_MEDIAN_HOUSEHOLD_INCOME_MHI_RISK_LEVEL_BI == "HIGH" ~ 1,
        ...
        FUNDING_any_failingoratriskonly = case_when(FUNDING_any_failingoratriskonly == "No" ~ 1,
...
TMF <- Data %>% 
  dplyr::select(PWSID, OPERATOR_CERTIFICATION_VIOLATIONS_RISK_LEVEL, MONITORING_AND_REPORTING_VIOLATIONS_RISK_LEVEL, DAYS_CASH_ON_HAND_RISK_LEVEL,
                              OPERATING_RATIO_RISK_LEVEL) %>%
  mutate(OPERATOR_CERTIFICATION_VIOLATIONS_RISK_LEVEL = case_when(OPERATOR_CERTIFICATION_VIOLATIONS_RISK_LEVEL == "HIGH" ~ 1,
        ...
        MONITORING_AND_REPORTING_VIOLATIONS_RISK_LEVEL = case_when(MONITORING_AND_REPORTING_VIOLATIONS_RISK_LEVEL == "HIGH" ~ 1,
        ...
        OPERATING_RATIO_RISK_LEVEL = case_when(OPERATING_RATIO_RISK_LEVEL == "HIGH" ~ 1,
...
#make first version five level factors
...
levels(WQ$WQ_combined_count) <- c(levels(WQ$WQ_combined_count), "3" ,"4")
...
write.csv(Data, file = here::here("Data_processed/Compiled_data_July2024.csv"))
